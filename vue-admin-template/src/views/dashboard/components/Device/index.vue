<template>
  ...
  <el-card id="card" v-bind:class="[isConnected? 'device-card':'device-card card-disconnect']" :body-style="{ padding: '0px' }">
    <div v-bind:class="[isConnected? 'card-title':'card-title title-disconnect']">
    ...
    <el-row>
      <el-col class="test" :span="18">
         <div class="text">
          ...
          <span>心率</span>
          <span style="float: right">{{ device.vitalSigns.hr }}
        ...
        <div class="text">
          ...
          <span v-if="device.vitalSigns.spo2!=undefined & device.vitalSigns.spo2<spo2_thr" class="spo2-enhance">血氧濃度</span>
          <span v-else>血氧濃度</span>
          
          <span v-if="device.vitalSigns.spo2!=undefined & device.vitalSigns.spo2<spo2_thr" 
          style="float: right" class="spo2-enhance">{{ device.vitalSigns.spo2 }}
              <span style="color: gray">％</span>
          </span>
          <span v-else  style="float: right">{{ device.vitalSigns.spo2 }}
            <span style="color: gray">％</span>
          </span>
        </div>
        <div class="text">
  ...
  methods: {
    goToInfoPage() {
      if (this.device.resident) {
        this.$router.push({ path: this.redirect || "/infomation/index", query:{ residentId: this.device.resident._id }});
      }
    },
    goRecordPage(){
